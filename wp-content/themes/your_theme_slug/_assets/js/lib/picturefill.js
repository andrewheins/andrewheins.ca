...
// ADDED By Cantilever 7/4/14 – Support for hard-coded height and width attributes to aid in centering
if (firstMatch.getAttribute("height")) { picImg.height = firstMatch.getAttribute("height"); }
if (firstMatch.getAttribute("width")) { picImg.width = firstMatch.getAttribute("width"); }
...
// ADDED By Cantilever 7/4/14 – Fire a special event here so we can react to that with centering functions, etc instead of hoping that the order of resize events helps us out
if (window.jQuery) {
	$(picImg).trigger("picturefill");
}
